...
#import "BinaryTreePrinter.h"

@implementation BinaryTreePrinter

+ (void)printTreeAtRoot:(TreeNode *)node {
    if (node == nil) {cout << "empty tree";return;}
    __block __weak BinaryTree*(^weak_objc2cpp)(TreeNode*);
    BinaryTree*(^objc2cpp)(TreeNode*);
    weak_objc2cpp = objc2cpp = ^BinaryTree*(TreeNode *objcNode) {
        BinaryTree *cppNode = new BinaryTree(int(objcNode.data));
        if (objcNode.left != nil) {cppNode->left = weak_objc2cpp(objcNode.left);}
        if (objcNode.right != nil) {cppNode->right = weak_objc2cpp(objcNode.right);}
        return cppNode;
    };
    BinaryTree *root = objc2cpp(node);
    cout << "Tree pretty print\n\n";
